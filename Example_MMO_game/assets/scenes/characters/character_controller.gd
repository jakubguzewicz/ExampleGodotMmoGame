...
func change_equipped_item(index: int):
	if is_instance_valid(player_character.weapon):
...
func _input(event):
	...
	if event is InputEventMouseMotion:
		...
		var mouse_position_absolute: Vector2 = (event.position - viewport_size / 2)
		var mouse_position_relative: Vector2 = mouse_position_absolute / (viewport_size/2)
		var rotation : float = -(mouse_position_absolute).angle_to(Vector2.UP)
		player_character.rotation = rotation
		camera.global_position = player_character.global_position + mouse_position_relative * CAMERA_MOVEMENT_RATIO
		#var camera_distance = mouse_position_relative.rotated(mouse_position_absolute.angle_to(Vector2.UP)) * CAMERA_MOVEMENT_RATIO
		#print(camera.position)
		#camera.position = camera_distance
