...
func _process_client_update(character_update_dict: Dictionary):
	...
	if updated_character_array.size() != 0:
		...
		print("Action data: "+str(actions_data))
		print(actions_data.keys())
		...
		for action in actions_data.keys():
			...
			match action:
				...
				ActionTypes.Types.CHANGE_ITEM:
					_change_equipped_item(updated_character, actions_data[ActionTypes.Types.CHANGE_ITEM])
	else:
		pass
...
func _add_character(character_data: Array):
	...
	new_character.equipment.item_dropped.connect(_item_dropped)
	_change_equipped_item(new_character, 0)
	
func _item_dropped(item, pickup_transform:=Transform2D()):
	interactables_node.add_interactable(item, pickup_transform)
