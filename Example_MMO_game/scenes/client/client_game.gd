			...
			if data.size() == 3:
				for index in controlled_character.equipment.equipment_array.size():
					if data[2][index] != controlled_character.equipment.equipment_array[index].equipment_id:
						controlled_character.equipment.replace_item(index, EquipmentLibrary.weapon_scenes[data[2][index]].instantiate())
						controlled_character.player_character.change_weapon(controlled_character.equipment.change_selected_item(index))
		else:
